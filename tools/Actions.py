...
class Actions(Enum):
    ...
    Guideline_Search = "Guideline Search"
...
def get_action_results(
    ...
    if result_string == f"{action.value}:\n":
        result_string = "No results available. Please try another Action."
    ...
    for rad in action_results["Radiology"]:
        if (
            ...
            if repeat_scan_index == 0:
                ...
                repeat_scan_index -= 1
    if not result:
        available_scans = [{'region': test['Region'], 'modality': test['Modality']} for test in action_results["Radiology"]]
        result = f"Not available. Try a different imaging modality. Available options: {json.dumps(available_scans)}"
        result = f"Not available. Try a different imaging modality."
