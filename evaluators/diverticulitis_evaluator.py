...
class DiverticulitisEvaluator(PathologyEvaluator):
    ...
    def generate_feedback(self) -> str:
        """
        Diverticulitis specific feedback:
        - Start from generic feedback (no treatment).
        - Add diverticulitis specific imaging and treatment comments.
        """
        ...
        has_ct_abd = any(
            ...
            and item.get("modality") == "CT"
            for item in correct_imaging
        )
        has_us_mri_abd = any(
            isinstance(item, dict)
            and item.get("region") == "Abdomen"
            and item.get("modality") in ("Ultrasound", "MRI")
            for item in correct_imaging
        )

        if imaging_score == 1:
            if has_us_mri_abd and not has_ct_abd:
                messages.append(
                    "Imaging (diverticulitis): set region='Abdomen' and use CT as first line. Ultrasound or MRI are also recommended as additional follow up options."
                )
            else:
                messages.append(
                    "Imaging (diverticulitis): abdominal imaging was partially appropriate. Prefer set region='Abdomen' with CT as first line, with ultrasound or MRI as additional follow up options."
                )
        elif imaging_score == 0:
            messages.append(
                "Imaging (diverticulitis): no appropriate abdominal imaging was done (set region='Abdomen' and request CT/ultrasound/MRI). Prefer CT as first line, with ultrasound or MRI as additional follow up options."
            )
